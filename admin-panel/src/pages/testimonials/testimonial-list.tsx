...
const columns: ColumnDef<Testimonial, unknown>[] = [
  ...
  { accessorKey: 'author', header: 'Автор', size: 150 },
  { accessorKey: 'rating', header: 'Рейтинг', size: 80 },
  {
...
export default function TestimonialListPage() {
  ...
  const { page, setPage, search, setSearch, deleteId, setDeleteId, data, isLoading, deleteMutation } =
    useCrudList(testimonialsApi, {
      queryKey: 'testimonials',
      deleteSuccessMessage: 'Отзыв удалён',
    })

  const actionColumns: ColumnDef<Testimonial, unknown>[] = [
    ...columns,
    {
      id: 'actions',
      cell: ({ row }) => (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => navigate('/panel/testimonials/new')}>
      ...
      <DataTable
        ...
        onRowClick={(row) => navigate(`/panel/testimonials/${row.id}`)}
      />
